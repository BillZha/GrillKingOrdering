...
def epson_direct_print(request):
    connection_type = request.POST.get("ConnectionType", "")
    printer_id = request.POST.get("ID", "")

    if printer_id != "grillking-kitchen":
        return HttpResponse("", status=403)

    # Epson asks Render for a new print job.
    if connection_type == "GetRequest":
        ...
        order = (
            ...
            .filter(
                models.Q(direct_print_sent_at__isnull=True)
                | models.Q(direct_print_sent_at__lte=retry_before)
            )
            .order_by("created_at")
            .first()
        )
        ...
        order.save(update_fields=["direct_print_sent_at"])

        job_id = f"order-{order.id}"
        table_number = str(order.table_number).zfill(2)
        ...
        for item in order.items.select_related("menu_item").all():
            ...
            if has_chinese_name:
                kitchen_name = item.menu_item.name_zh
                item_line = f"{quantity} × {kitchen_name}"

                if spicy and spicy != "Not Spicy":
                    item_line += "  （辣）"

                try:
                    items_xml += text_to_epos_image(item_line)
                    items_xml += '<feed line="1"/>'
                except Exception as error:
                    # Do not let a font/image problem stop kitchen printing.
                    print("Chinese kitchen image error:", error)

                    safe_name = saxutils.escape(str(item.name))
                    items_xml += (
                        '<text width="1" height="2"/>'
                        '<text em="true"/>'
                        f'<text>{quantity} x {safe_name}&#10;</text>'
                    )

                    if spicy and spicy != "Not Spicy":
                        safe_spicy = saxutils.escape(str(spicy).upper())
                        items_xml += (
    ...
    # Epson sends the print result back to Render.
    if connection_type == "SetResponse":
        response_file = request.POST.get("ResponseFile", "")
        ...
        try:
            ...
            for element in root.iter():
                ...
                if tag == "printjobid":
                    job_id = element.text or ""

                if tag == "response":
                    success = (
                        element.attrib.get("success") == "true"
                    )

            if job_id and job_id.startswith("order-"):
                order_id = int(job_id.replace("order-", ""))
                ...
                if order:
                    if success:
                        ...
                        order.save(
                            update_fields=["direct_printed"]
                        )
                    else:
                        # Allow the same job to be retried if Epson reports failure.
                        order.direct_print_sent_at = None
                        order.save(
                            update_fields=["direct_print_sent_at"]
                        )

        except Exception as error:
            print("Epson SetResponse error:", error)
